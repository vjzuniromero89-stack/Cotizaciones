...
export default function ProfitPage(){
 ...
 const confirmedProfit=confirmed.reduce((s,x)=>s+Number(x.total)*.30,0),potentialProfit=potential.reduce((s,x)=>s+Number(x.total)*.30,0),confirmedSales=confirmed.reduce((s,x)=>s+Number(x.total)*1.30,0);
 ...
 const maxProfit=Math.max(1,...visible.map(x=>Number(x.total)*.30));
 return <div className="workspace profitPage">
  ...
  <section className="profitPanel"><div className="profitControls"><div className="profitFilters"><button className={filter==='all'?'active':''} onClick={()=>setFilter('all')}>Todas</button><button className={filter==='confirmed'?'active':''} onClick={()=>setFilter('confirmed')}>Confirmadas</button><button className={filter==='potential'?'active':''} onClick={()=>setFilter('potential')}>Pendientes</button></div><label><Search/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar cliente o cotización…"/></label></div>
   {loading?<div className="profitEmpty">Calculando ganancias…</div>:visible.length===0?<div className="profitEmpty"><FileText/><b>No hay resultados</b><span>Las ganancias aparecerán cuando guardes cotizaciones.</span></div>:<div className="profitList">{visible.map(x=>{const cost=Number(x.total),sale=cost*1.30,profit=cost*.30;return <article className="profitRecord" key={x.id}><div className="profitRecordTop"><div><span className={'profitStatus '+x.profitState}>{x.profitState==='confirmed'?<><CheckCircle2/> Ganancia confirmada</>:<><Clock3/> Ganancia potencial</>}</span><b>{x.number}</b><h3>{x.customer_name}</h3><p>{x.description||'Sin descripción'} · {x.total_boxes} cajas</p></div><div className="profitAmount"><small>GANANCIA</small><strong>+{money(profit)}</strong><span><ArrowUpRight/> 30% sobre el costo</span></div></div><div className="profitBar"><i style={{width:`${profit/maxProfit*100}%`}}/></div><div className="profitBreakdown"><div><span>Costo interno</span><b>{money(cost)}</b></div><div><span>Cobrado al cliente</span><b>{money(sale)}</b></div><div><span>Fecha</span><b>{new Date(x.created_at).toLocaleDateString('es-NI')}</b></div></div></article>})}</div>}
  </section>
 </div>
}
