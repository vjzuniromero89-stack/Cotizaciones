...
describe("flujo de productos y cotizaciones", () => {
  ...
  it("guarda el cálculo primero como producto", async () => {
    ...
    const response = await worker.fetch(
      new Request("https://cotizaciones.test/api/products", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          selectedRoute: "miami",
          result,
          products: [{ name: "Botella", qty: 100, price: 1 }],
          boxes: [],
          rates: {},
        }),
      }),
  ...
  it("crea la cotización con precios de venta y ganancia", async () => {
    const source = {
      id: "p1",
      record_type: "product",
      customer_name: "Botellas",
      description: "100 botellas",
      products: [{ name: "Botella", qty: 100, price: 1 }],
      boxes: [],
      rates: {},
      totals: { totalUnits: 100 },
      route: "miami",
      total: 150,
      total_boxes: 2,
      notes: "",
    ...
    vi.stubGlobal("fetch", fetchMock);
    const response = await worker.fetch(
      new Request("https://cotizaciones.test/api/products/p1/quote", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          customer: "Wendy",
          products: [{ name: "Botella", qty: 100, price: 1, salePrice: 2 }],
        }),
      }),
      env,
    );
  ...
  it("guarda desde la calculadora una cotización interna y para cliente", async () => {
    ...
    const response = await worker.fetch(
      new Request("https://cotizaciones.test/api/quotes", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          customer: "Wendy",
          ...
          result,
          products: [
            { name: "Bolso", qty: 100, price: 1, salePrice: 3, boxes: [] },
          ],
          boxes: [],
          rates: {},
        }),
