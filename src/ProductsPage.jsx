...
function ProductDetail({id,onClose,onChanged}){
 ...
 return <><div className="overlay" onMouseDown={e=>e.target===e.currentTarget&&onClose()}><section className="detailSheet productRecordDetail">
  ...
  <div className="detailSection"><h3><Package/> Información de cajas ({data.total_boxes||0})</h3>{boxes.length?<div className="productBoxTable linkedBoxes"><div className="productBoxRow head"><span>Producto</span><span>Cant.</span><span>Medidas</span><span>Peso/caja</span><span>CBM/caja</span><span>CBM total</span></div>{boxes.map((b,i)=>{const cbm=boxCbm(b),qty=Math.max(0,Number(b.qty)||0);return <div className="productBoxRow" key={b.id||i}><b className="boxProductName">{b.productName||data.products?.find(p=>p.id===b.productId)?.name||'Producto'}</b><b>{qty}</b><span>{b.l} × {b.w} × {b.h} {b.unit}</span><span>{b.weight} {b.weightUnit}</span><span>{cbm.toFixed(4)}</span><b>{(cbm*qty).toFixed(4)}</b></div>})}</div>:<p className="boxEmpty">Este producto no tiene cajas registradas.</p>}</div>
